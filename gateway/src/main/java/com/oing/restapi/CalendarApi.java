...
public interface CalendarApi {
    ...
    @Operation(summary = "일간 캘린더 조회", description = "일간 캘린더를 조회합니다.\n\n 주어진 날에 업로드된 게시글들이 조회되며 정책에 따라, 게시일자 오름차순 && 생존신고 게시글 → 미션 게시글 순서로 정렬됩니다.")
    @GetMapping("/daily")
    ...
    @Operation(summary = "[Deprecated] 월별 캘린더 조회", description = "[Deprecated]\n\n 월별로 캘린더를 조회합니다. 각 날짜의 대표 게시글 정보와 가족 구성원 전부의 업로드 여부가가 조회되며, 해당 날짜에 게시글이 있는 경우만 response 에 포함됩니다.")
    @GetMapping(params = {"type=MONTHLY"})
    ArrayResponse<MonthlyCalendarResponse> getMonthlyCalendarLegacy(
            @RequestParam
            @Parameter(description = "조회할 년월", example = "2021-12")
            String yearMonth,
