...
public interface MemberPostRepository extends JpaRepository<MemberPost, String> {
    ...
    List<MemberPost> findLatestPostOfEveryday(List<String> memberIds, LocalDate startDate, LocalDate endDate);
    ...
    @Query("SELECT new com.oing.domain.MemberPostCountDTO(p.createdAt, COUNT(p)) FROM member_post p " +
            ...
            "ORDER BY p.createdAt ASC")
    List<MemberPostCountDTO> countPostsOfEveryday(List<String> memberIds, LocalDate startDate, LocalDate endDate);
}
